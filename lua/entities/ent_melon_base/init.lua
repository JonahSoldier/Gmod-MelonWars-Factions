...
function MelonWars.defaults( ent )
	...
	ent.mw_networkFilter = RecipientFilter( true )
end
...
cvars.AddChangeCallback( "mw_admin_network_performance_mode", function(convName, oldVal, newVal)
	if not tobool(newVal) then
		hook.Remove("Tick", "MelonWars_experimental_net_optimization")
		...
		for i, v in ipairs(ents.GetAll()) do
			...
			if vTbl.Base == "ent_melon_base" then
				...
				vTbl.mw_networkFilter:RemoveAllPlayers()
			end
		end
	...
	local nextThink = 0
	hook.Add("Tick", "MelonWars_experimental_net_optimization", function()
		if CurTime() < nextThink then return end
		nextThink = nextThink + 0.1
		...
		for i, v in ipairs(ents.GetAll()) do
			...
			if IsValid(v) and vTbl.Base == "ent_melon_base" and vTbl.mw_networkFilter then
				...
				vTbl.mw_networkFilter:RemoveAllPlayers()
				vTbl.mw_networkFilter:AddPVS( v:GetPos() )
			end
		end
